...
3. Run `gh issue list --state open` to check for duplicates. If a related issue exists, report it.
4. Read `.github/ISSUE_TEMPLATE/feature_request.yml` if it exists to get the exact field structure. Otherwise use a standard feature request format (problem/motivation, proposed solution, acceptance criteria).
